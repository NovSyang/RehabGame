...
import { connectionManager, persistTrainingResult, sensorService } from '../app/AppServices'
import type { GameHudSnapshot } from '../core/game/TrainingGameEvents'
import type { ITrainingGame } from '../core/game/ITrainingGame'
import { pauseReasonAfterDisconnect, shouldAutoResume, type PauseReason } from '../core/game/TrainingPausePolicy'
...
<template>
  <main class="training-shell">
    <header class="training-toolbar">
      ...
      <div class="row"><button class="button" :disabled="!canPause" @click="togglePause">{{ trainingState === 'paused' ? '继续' : '暂停' }}</button><button class="button danger" @click="abort">结束训练</button></div>
    </header>
