...
<template>
  <main class="shell">
    <aside class="panel">
      ...
      <section class="card"><div class="section-title">个人活动范围</div><p class="muted small">{{ hasMeasuredProfile ? '已保存个人活动范围。' : '首次使用需要完成四方向个人活动范围测量。' }}</p><button class="button" :disabled="!connected || !snapshot.gameInput.calibrated" @click="router.push('/rom-calibration')">{{ hasMeasuredProfile ? '重新测量' : '开始测量' }}</button></section>
