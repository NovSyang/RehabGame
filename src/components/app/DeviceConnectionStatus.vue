...
// Compact 仅收紧训练 HUD 的文字，设备管理菜单行为保持一致。
...
const statusAriaLabel = computed(() => {
  const state = connected.value ? '设备已连接' : '设备未连接'
  return props.compact && connected.value ? `${state}，${batteryAriaLabel.value}` : state
})
...
<template>
  <div ref="root" class="device-status-menu">
    <button
      class="device-status"
      :class="{ 'device-status--compact': compact }"
    ...
    >
      ...
      <span v-if="!compact">{{ connected ? '设备已连接' : '设备未连接' }}</span>
      <span v-if="connected" class="battery-status" :data-low="batteryLow" :aria-label="batteryAriaLabel">
    ...
    <div v-if="open" class="device-status-dropdown" role="menu">
      <button class="dropdown-item" :disabled="!connection.binding" @click="reconnect">重新连接</button>
      <button class="dropdown-item" @click="switchDevice">更换设备</button>
      <button class="dropdown-item danger-text" :disabled="!connection.binding" @click="forget">忘记设备</button>
    </div>
    <p v-if="errorMessage" class="device-status-error">{{ errorMessage }}</p>
    <DeviceSwitchDialog v-if="showSwitchDialog" @close="showSwitchDialog = false" @connected="showSwitchDialog = false" />
  </div>
</template>
